...
### Blender Addon (`addon/`)
- `__init__.py` — Registers the Blender extension: properties, operators, UI panel in Camera Properties. Each camera gets its own text datablock (`Physical Lens - {cam.name}`) containing a shader with only the selected lens. On register, loads the OSL template and lens registry; shaders are generated per-camera on lens selection or file load
...
- `codegen.py` — `load_osl_template()` loads the template and lens registry; `build_camera_osl()` injects a single lens's data and scene lights into the template to produce a final shader. Generates per-lens `#define`s (`NUM_SURFACES`, `COATING`, `SQUEEZE`, `FOCUS_CLOSE_DISTANCE`) and a `load_lens_data()` function that outputs per-surface arrays. Static defines (`MAX_SURFACES`, `SURFACE_*` types) live in the template
- `scene_lights.py` — Collects Blender LIGHT objects and emissive meshes in world space, and generates the `load_scene_lights()` OSL function. Positions are in world meters; sun directions are world-space "toward source" vectors. Up to `MAX_LIGHTS` (16) lights sorted by intensity. Camera-independent — camera movement does not trigger shader recompilation
...
1. `load_lens_data()` — generated per-camera with a single lens's data. Outputs per-surface arrays: `surface_types[]` (int per surface: `SURFACE_SPHERICAL=0`, `SURFACE_FLAT=1`, `SURFACE_STOP=2`, `SURFACE_ASPHERIC=3`, `SURFACE_CYLINDRICAL_X=4`, `SURFACE_CYLINDRICAL_Y=5`), `extra[]` (8 floats per surface: k, A4, A6, A8, A10, reserved×3; only non-zero entries emitted), `thicknesses_close[]`. Per-lens scalars are compile-time `#define`s: `NUM_SURFACES`, `COATING`, `SQUEEZE`, `FOCUS_CLOSE_DISTANCE`
1b. `load_scene_lights()` — generated on depsgraph updates; provides scene light positions/directions/intensities in world space for ghost light-aiming. The shader calls `transform_lights_to_camera()` to convert world-space data to camera/lens space at render time, so camera movement doesn't require shader recompilation. Light data is cached; the shader is only regenerated when lights actually change
1c. `transform_lights_to_camera()` — converts world-space light positions (meters) and sun directions to camera space (mm) using OSL's `transform("world", "camera", ...)`. Called after `load_scene_lights()` in both debug mode 5 and the main ghost path
1d. `rng()` — integer hash (Jenkins one-at-a-time) that derives decorrelated random values from the two camera random samples. Each salt value yields an independent uniform [0,1] variate, replacing the earlier sin()-based hashes
...
10. `reflect_at_surface()` — calls `find_surface_intersection()`, then applies `reflect()` + Fresnel reflectance. Used for ghost bounces (never called on stops)
11. `trace_surface_range()` — traces a contiguous range of surfaces in either direction (step=±1). Refracts at each surface except an optional reflect_at_idx where it reflects. No sphere-overlap retry logic
12. `trace_ghost_path()` — composes three `trace_surface_range()` calls for a double-bounce ghost path: (a) rear→bounce_a with reflect, (b) bounce_a+1→bounce_b with reflect, (c) bounce_b-1→front with refract
13. `check_aperture_at_plane()` — projects a ray to a z-plane and checks circular or n-gon aperture clip
14. `refract_at_flat_plane()` — Snell's law at a flat surface with Fresnel transmittance
