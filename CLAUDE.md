...
- `codegen.py` — `load_osl_template()` loads the template and lens registry; `build_camera_osl()` injects a single lens's `load_lens_data()` and scene lights into the template to produce a final shader. Emits `surface_types[]`, `extra[]`, `thicknesses_close[]` arrays, `focus_close_distance`, `squeeze`, and `SURFACE_*` type constants
